...
async def main():
    ...
    asyncio.create_task(websocket.start_websocket_task_data_async(type='user-position'
                                                             , data=user_position
                                                             , request_wait_time=request_wait_time))
    ''' type: user-satellite-connection '''
    ...
    '''
    asyncio.create_task(websocket.const_position(type='beijing', position=[116.2, 39.56], request_wait_time=request_wait_time))



    await asyncio.Event().wait()  # 等待永远不会发生的事件
    return
